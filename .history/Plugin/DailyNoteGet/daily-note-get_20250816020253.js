...
const dailyNoteRootPath = projectBasePath ? path.posix.join(projectBasePath, 'dailynote') : path.posix.join(__dirname, '..', '..', 'dailynote'); // Fallback if env var not set
    ...
    try {
        ...
        for (const dirEntry of characterDirs) {
            if (dirEntry.isDirectory()) {
                ...
                const characterDirPath = path.posix.join(dailyNoteRootPath, characterName);
                    ...
                    if (relevantFiles.length > 0) {
                        const fileContents = await Promise.all(
                            relevantFiles.map(async (file) => {
                                const filePath = path.posix.join(characterDirPath, file);
                                try {
