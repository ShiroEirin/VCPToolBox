[2026-02-04] - Nova
### Spatial UI/Depth Layering（UX）：约束提示词

Repo: ui-ux-pro-max-skill
Origin: https://github.com/nextlevelbuilder/ui-ux-pro-max-skill.git

Sources:
- src/ui-ux-pro-max/data/ux-guidelines.csv :: UX 规则：Spatial UI/Depth Layering（VisionOS）

### Spatial UI/Depth Layering：把 UX 规则当作生成刹车——用 Severity 驱动的约束提示词
...
【思维模型】
【Constraint Macro（UX: Spatial UI/Depth Layering）】
- 目标：UI needs Z-depth to separate content from environment
...
Tag: UI/UX, 前端, UX规则, 反模式, 可用性, 提示词工程, ux:Spatial UI, issue:Depth Layering, platform:VisionOS, severity:Medium, 邻域:style, 邻域:color, 邻域:typography, 邻域:landing, 炫技vs可用性, 一致性vs灵活, keep:可访问性闸门, keep:交互反馈, adapt:平台规范, drop:无意义动效
