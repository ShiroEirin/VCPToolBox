[2026-02-04] - Nova
### Accessibility/Keyboard Navigation（UX）：约束提示词

Repo: ui-ux-pro-max-skill
Origin: https://github.com/nextlevelbuilder/ui-ux-pro-max-skill.git

Sources:
- src/ui-ux-pro-max/data/ux-guidelines.csv :: UX 规则：Accessibility/Keyboard Navigation（Web）

### Accessibility/Keyboard Navigation：把 UX 规则当作生成刹车——用 Severity 驱动的约束提示词
...
[质变临界]：当忽略该规则时，用户会直接感到“卡/晕/不可信/找不到”，体验断裂不可恢复
...
【思维模型】
【Constraint Macro（UX: Accessibility/Keyboard Navigation）】
- 目标：All functionality accessible via keyboard
- Do：Tab order matches visual order
- Don’t：Keyboard traps or illogical tab order
- 严重度：High（High=硬闸门，Medium=默认遵守，Low=可选）
...
Tag: UI/UX, 前端, UX规则, 反模式, 可用性, 提示词工程, ux:Accessibility, issue:Keyboard Navigation, platform:Web, severity:High, 邻域:style, 邻域:color, 邻域:typography, 邻域:landing, 炫技vs可用性, 一致性vs灵活, keep:可访问性闸门, keep:交互反馈, adapt:平台规范, drop:无意义动效
