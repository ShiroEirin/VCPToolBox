[2026-02-04] - Nova
### Server/React.cache Dedup（React Perf）：约束提示词

Repo: ui-ux-pro-max-skill
Origin: https://github.com/nextlevelbuilder/ui-ux-pro-max-skill.git

Sources:
- src/ui-ux-pro-max/data/react-performance.csv :: React 性能：Server/React.cache Dedup

### Server/React.cache Dedup：性能不是优化，是设计——把 React 性能经验编译成约束提示词
...
【思维模型】
【Constraint Macro（React Perf: Server/React.cache Dedup）】
- 症状/本质：Use React.cache() for server-side request deduplication within single request
...
Tag: 前端, React, 性能, 提示词工程, 质量闸门, react:Server, issue:React.cache Dedup, severity:Medium, 邻域:ux, 邻域:web, 邻域:nextjs, 性能vs可维护, 抽象vs具体, keep:渲染边界, keep:依赖稳定, adapt:数据获取策略, drop:无脑重渲染
